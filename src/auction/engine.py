...
class AuctionEngine:
    ...
    async def accept_bid(self, auction_id: str, robot_name: str) -> AuctionResult:
        ...
        if self._payment_handler is not None:
            try:
                ...
                logger.error("Failed to create Stripe checkout for auction %s: %s", auction_id, exc)
    ...
    async def on_payment_confirmed(self, auction_id: str) -> None:
        ...
        Marks the auction as 'paid'. If the winning robot has
        requires_approval=False (Mode B), task execution is scheduled
        automatically via asyncio.create_task(). Otherwise (Mode A), the
        operator must call fleet_execute_task manually.
        """
        auction = self._get_auction(auction_id)
        auction.status = "paid"
        ...
        if not requires_approval:
            ...
            asyncio.create_task(self.execute(auction_id))
